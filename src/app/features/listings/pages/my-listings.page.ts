...
export class MyListingsPage {
  ...
  private readonly statusTextKeys: Record<ListingStatus, string> = {
  ...
  readonly statusFilters: ListingStatusFilter[] = [
    'all',
    'pending_review',
    'draft',
    'published',
  ...
  listingStatusLabel(status: ListingStatus): string {
    return this.text(this.statusTextKeys[status]);
  }
  ...
  showModerationReason(listing: Listing): boolean {
    return Boolean(listing.moderation_reason) && ['blocked', 'rejected'].includes(listing.status);
  }
  ...
  private sortListings(listings: Listing[]): Listing[] {
    ...
    if (this.sortOption() === 'oldestUpdated') {
      return sortedListings.sort((first, second) => {
        return this.compareUpdated(first, second);
      });
    }

    if (this.sortOption() === 'priceHighToLow') {
      return sortedListings.sort((first, second) => {
        return this.comparePrice(second, first);
      });
    }

    if (this.sortOption() === 'priceLowToHigh') {
      return sortedListings.sort((first, second) => {
        return this.comparePrice(first, second);
      });
    }
  ...
  private sortByTextOrRecent(listings: Listing[]): Listing[] {
    if (this.sortOption() === 'titleAz') {
      return listings.sort((first, second) => {
        return this.compareTitle(first, second);
      });
    }

    if (this.sortOption() === 'titleZa') {
      return listings.sort((first, second) => {
        return this.compareTitle(second, first);
      });
    }

    return listings.sort((first, second) => {
      return this.compareUpdated(second, first);
    });
  }
