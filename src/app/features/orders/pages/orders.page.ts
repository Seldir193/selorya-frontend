...
@Component({
  ...
  imports: [RouterLink, DropdownComponent, PaginationComponent, OrderDetailDialogComponent],
...
export class OrdersPage {
  ...
  readonly statusDropdownOptions = computed<DropdownOption<OrderStatusFilter>[]>(() =>
    this.createStatusOptions(),
  );

  readonly sortDropdownOptions = computed<DropdownOption<OrderSortOption>[]>(() =>
  ...
  orderDateOnly(order: Order): string {
    return formatDisplayDateOnly(order.created_at, this.i18n.current());
  }

  paymentSummary(order: Order): string {
    return [
      this.paymentProviderLabel(order.payment_provider),
  ...
  mainItemTitle(order: Order): string {
    return order.items[0]?.title_snapshot ?? this.text('ordersUnknownItem');
  }

  private createStatusOptions(): DropdownOption<OrderStatusFilter>[] {
  ...
  private createSortOptions(): DropdownOption<OrderSortOption>[] {
